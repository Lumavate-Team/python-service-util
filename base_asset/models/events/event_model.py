from app import db
from flask import g
from sqlalchemy import ForeignKey, select, delete, cast, and_, or_, union, Text, case
...
from ...models.events.events_related_products import EventsRelatedProductsModel
from ...models.events.event_type import EventTypeModel
...
class EventModel(BaseModel):
  ...
  def get_all_by_asset_id(cls, asset_id, args=None):
    query = cls.get_all(args).filter_by(event_type_id=asset_id)
  ...
  def get_all_by_asset_id_activation(cls, asset_id):
    if g.token_data.get('code', None):
      return cls.get_all().filter_by(event_type_id=asset_id,activation_code=g.token_data.get('code', None))
  ...
  def get_by_public_id(cls, asset_id, public_id):
    return cls.get_all().filter_by(event_type_id=asset_id, public_id=public_id).first()
