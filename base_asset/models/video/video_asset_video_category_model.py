...
class VideoAssetVideoCategoryModel(BaseModel):
  ...
  def get_categories_by_assets(cls, asset_ids):
    return VideoCategoryModel.get_by_ids(cls.query.with_entities(VideoAssetVideoCategoryModel.category_id).filter(and_(cls.org_id==g.org_id, cls.asset_id.in_(asset_ids))).all())
  
  @classmethod
  def get_categories_by_type_and_assets(cls, type, asset_ids):
    return cls.query.with_entities(VideoCategoryModel, VideoAssetVideoCategoryModel).filter(and_(cls.org_id==g.org_id, cls.asset_id.in_(asset_ids))).\
      join(VideoCategoryModel, and_(VideoCategoryModel.id == cls.category_id, VideoCategoryModel.type==type))
  ...
  def get_all_by_ids(cls, category_ids):
    return cls.query.filter(and_(cls.org_id==g.org_id)).\
      join(VideoCategoryModel, and_(VideoCategoryModel.id == cls.category_id, VideoCategoryModel.id.in_(category_ids)))
