...
class CategoryModel(BaseModel):
  ...
  def get_by_type(cls, id, type):
    return cls.get_all_by_type(type).filter_by(old_id=id).first()

  @classmethod
  def get_by_ids(cls, ids):
    return cls.get_all().filter(cls.old_id.in_(ids))

  @classmethod
  def get_by_ids_and_type(cls, ids, type):
    return cls.get_all().filter(and_(cls.old_id.in_(ids), cls.type==type))
